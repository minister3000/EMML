...
LogLikeliHood_FULL_R <- function(
  ...
  if(n_transition == 2){
    ...
    help1       <-  exp(trans_p_explanatory %*% (cbind(theta_[1],  theta_[2])))
  }
  ...
  if(printLogLike) cat(paste("Log-likelihood for provided parameters 'param' is:", sprintf("%.5f", sum(like_t_T_temp))))

  return(list(  like_t_T_temp                =  like_t_T_temp,      # individual observation's log likelihood contribution
                LogLikelihoodVal             =  sum(like_t_T_temp), # sum of all log likelihood contributions
